...
.. note:: Macro effects can't contain zerary effect nodes in them (i.e. any effects in the Background or Render categories). If any zerary effect node is selected when trying to use the Make Macro FX command, no macro effect will be created.
